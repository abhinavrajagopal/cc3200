...
/*
 * Set up the hardware ready to run */
...
void vApplicationMallocFailedHook( void )
{
	...
	configUSE_MALLOC_FAILED_HOOK is set to 1 in Config.h.  It is a hook
	function that will get called if a call to pvPortMalloc() fails.
	pvPortMalloc() is called internally by the kernel whenever a task, queue,
	timer or semaphore is created. It is also called by various parts of the application. 
    If heap_1.c or heap_2.c are used, then the size of the
	heap available to pvPortMalloc() is defined by configTOTAL_HEAP_SIZE in
	Config.h, and the xPortGetFreeHeapSize() API function can be used
...
void vApplicationIdleHook( void )
{
	...
	to 1 in Config.h.  It will be called on each iteration of the idle
